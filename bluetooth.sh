...
if [[ -z $list ]]; then
	dunstify 'Bluetooth' 'No devices found'
	exit 1
fi
...
if [[ $connected == 'yes' ]]; then
	dunstify 'Bluetooth' 'Already connected to this device'
	exit 0
fi
...
if [[ $paired == 'no' ]]; then
	...
	if ! timeout $s bluetoothctl pair "$address" >/dev/null; then
		dunstify 'Bluetooth' 'Failed to pair'
		exit 1
	fi
fi
...
if timeout $s bluetoothctl connect "$address" >/dev/null; then
	dunstify 'Bluetooth' 'Successfully connected'
else
